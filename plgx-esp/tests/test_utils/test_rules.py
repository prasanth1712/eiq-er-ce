...
from polylogyx.utils.rules import (BaseCondition, EqualCondition,
                                   GreaterEqualCondition, LessCondition,
                                   LogicCondition, MatchesRegexCondition,
                                   Network, NotMatchesRegexCondition,
                                   RuleInput)
...
class TestFunctional:
    ...
    def test_will_alert(self, node):
        ...
        alerts = network.process(good_input, node.to_dict())
        assert len(alerts) == 0

        alerts = network.process(bad_input, node.to_dict())
        assert sorted(alerts, key=lambda v: v) == [1, 2, 3]
