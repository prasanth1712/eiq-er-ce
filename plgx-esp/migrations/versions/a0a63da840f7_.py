...
def upgrade():
   ...
   FOR EACH STATEMENT EXECUTE PROCEDURE node_query_count();
    ...
    op.execute("COMMIT;")
    op.execute(
        "CREATE UNIQUE INDEX CONCURRENTLY idx_result_log_node_name_desc ON public.result_log USING btree(name, node_id, id DESC);"
    )
...
def downgrade():
    ...
    # op.drop_index('idx_result_log_node_name_desc', table_name='result_log')
