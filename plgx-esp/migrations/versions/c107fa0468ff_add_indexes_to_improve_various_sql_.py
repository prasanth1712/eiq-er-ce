...
def upgrade():
    ...
    op.create_index(
        "idx_distributed_query_task_node_id_status",
        "distributed_query_task",
        ["node_id", "status"],
        unique=False,
    )
    op.create_index(
        op.f("ix_file_path_tags_file_path.id"),
        "file_path_tags",
        ["file_path.id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_node_tags_node.id"), "node_tags", ["node.id"], unique=False
    )
    op.create_index(
        op.f("ix_pack_tags_pack.id"), "pack_tags", ["pack.id"], unique=False
    )
    op.create_index(
        op.f("ix_query_tags_query.id"), "query_tags", ["query.id"], unique=False
    )
    op.create_index(
        "idx_result_log_node_id_timestamp_desc",
        "result_log",
        ["node_id", sa.text(u"timestamp DESC")],
        unique=False,
    )
    op.create_index(
        "idx_status_log_node_id_created_desc",
        "status_log",
        ["node_id", sa.text(u"created DESC")],
        unique=False,
    )
...
def downgrade():
    ...
    op.drop_index(
        "idx_distributed_query_task_node_id_status", table_name="distributed_query_task"
    )
    ### end Alembic commands ###
