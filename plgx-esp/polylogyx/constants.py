...
class PolyLogyxConstants:
    ...
    EVENTS_MAPPING = {
        ...
        "win_remote_thread_events": 3,
        "win_process_open_events": 4,
        "win_removable_media_events": 5,
        "win_image_load_events": 6,
        "win_image_load_process_map": 7,
        "win_http_events": 8,
        "win_ssl_events": 9,
        "win_socket_events": 10,
        "win_dns_events": 11,
        "win_dns_response_events": 12,
...
class EventQueries:
    ...
    TOTAL_SOCKET_EVENTS_LINUX = (
        "select count(*) from result_log where name like '%socket_events%' and name not like '%win_socket_events%';"
    )
...
class UtilQueries:
    ...
    ALERT_RECON_QUERIES_JSON = {
        ...
        "live_queries": [{"name": "win_epp_table", "query": "select * from win_epp_table;"}],
    }

    UNSIGNED_JSON = {"sign_info": "Unsigned"}
    UNSIGNED_BINARY = (
        "SELECT count(*) FROM result_log WHERE name like '%win_image_load_events%' and columns @> '"
        + json.dumps(UNSIGNED_JSON)
        + "'; "
    )
