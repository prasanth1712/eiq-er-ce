from polylogyx.models import db, Alerts, ResultLog, Rule, Node, NodeQueryCount, AlertLog,AnalystNotes
...
def get_alert_source(source, start_date, end_date, node, rule_id, event_ids):
    qs = db.session.query(Alerts).filter(Alerts.source == source)\
        .filter(ModelStatusFilters.ALERTS_NON_RESOLVED)\
        .filter(ModelStatusFilters.HOSTS_ENABLED).join(Node, Alerts.node_id == Node.id)
    if node:
...
def edit_alerts_status_by_alert(alert_ids, status=False, verdict=None, comment=None):
    ...
    if status:
        ...
        comment = None
    for alert in alerts:
        alert.update(status=status, verdict=verdict_value, comment=comment)
