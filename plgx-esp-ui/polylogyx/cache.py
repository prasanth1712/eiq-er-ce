...
from polylogyx.models import Role, Rule, Node, Settings, Config, DefaultFilters, DefaultQuery, db, IOCIntel
from polylogyx.constants import IOC_COLUMNS, CacheVariables
from polylogyx.extensions import redis_client
from polylogyx.log_setting import get_log_level_setting
from polylogyx.utils import assemble_options


def get_log_level():
    log_level = get_a_setting('er_ui_log_level')
    if not log_level:
        log_level = "WARNING"
    return log_level
...
def refresh_log_level():
    ui_setting = "WARNING"
    er_setting = "WARNING"
    ui_settings = get_log_level_setting()
    er_settings = get_log_level_setting('er_log_level')
    if ui_settings:
        ui_setting = ui_settings.setting
    if er_settings:
        er_setting = er_settings.setting
    update_cached_setting('er_ui_log_level', ui_setting)
    update_cached_setting('er_log_level', er_setting)
...
def get_all_cached_roles_from_db():
    ...
    """
    data = redis_client.get(CacheVariables.user_roles)
    if data:
        ...
        redis_client.set(CacheVariables.user_roles, json.dumps(role_tuples_array))
        redis_client.expire(CacheVariables.user_roles, CacheVariables.default_cache_refresh_interval)
...
def get_host_dict(node):
    ...
    for column in ['enrolled_on', 'last_checkin', 'last_status', 'last_result', 'last_config', 'last_query_read',
                   'last_query_write']:
...
def remove_cached_host(node_id=None, node_key=None, host_identifier=None):
    ...
    if node_id or host_identifier:
        ...
        for node_key_cached, host_dict in hosts.items():
            if (node_id and host_dict['id'] == node_id) or (
                    host_identifier and host_dict['host_identifier'] == host_identifier):
...
def get_online_node_keys(check_in_interval):
    online_hosts = []
    all_cached_hosts = get_all_cached_hosts()
    for node_key, node_dict in all_cached_hosts.items():
        if node_dict.get('state') is not None and node_dict.get('state') == Node.ACTIVE:
            if node_dict.get('last_checkin') and (datetime.datetime.utcnow() - datetime.datetime.strptime(
                    node_dict.get('last_checkin'), "%Y-%m-%d %H:%M:%S.%f")) < check_in_interval:
                online_hosts.append(node_key)
    return online_hosts
    ...
    if all_node_alerts and all_node_alerts[0]:
        ...
        for rule_id, alert_id_dict in all_node_alerts.items():
            if isinstance(alert_id_dict['created_at'], str):
                alert_id_dict['created_at'] = datetime.datetime.strptime(alert_id_dict['created_at'],
                                                                         "%Y-%m-%d %H:%M:%S.%f")
    else:
        all_node_alerts = {}
