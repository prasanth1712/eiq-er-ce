...
private tagsValidation:TagsValidationHandler
  ...
  this.updateQuery= this.fb.group({
    name: ['', [Validators.required, Validators.maxLength(this.maxInputCharacters)]],
    sql:'',
...
if(this.queriesdata_data?.tags ){
  var data = this.queriesdata_data.tags.toString();
  let tagToArray = data.split(',').map(item => item.trim());
    this.queriesdata_data.tags = tagToArray.join("\n");
}
...
saveForm() {
let tags = '';
if(typeof this.f.tags.value === 'string'){tags = this.f.tags.value.trim();}
else{ tags = this.f.tags.value;}
...
let objQueryData = Object.assign({}, this.updateQueryObj)
if(!(this.tagsValidation.isEmpty(objQueryData["tags"] ))){
  var isValidTagObject;
  isValidTagObject = this.tagsValidation.getAllTags(objQueryData["tags"]);
  if(isValidTagObject){
    this.errorMessageTag = '';
    objQueryData["tags"] = isValidTagObject;
  }
  else {
    this.errorMessageTag = 'Accepts only ' + this.maxTagCharacters + ' alpha numeric characters with @._-'
    return
  }
}
Swal.fire({
...
this.commonapi.update_queries_in_query_api(this.id,objQueryData).subscribe((res: any) => {
this.result=res;
